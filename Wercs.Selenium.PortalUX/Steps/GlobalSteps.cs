...
namespace WERCSmart
{
	...
	public class GlobalSteps
	{
		...
		[StepDefinition(@"I login into the WERCSmart Portal - userSavedAs:(.*)")]
		public void GivenILoginAsSpecificRole(string savedAs)
		{
			TestReport.BeginTestModule(GlobalParameters.StepCount + " - Log into WERCSmart Portal as Specific Role");

			try
			{
				var Sel_Login = new Login();
				if (!Sel_Login.Wait_for_load())
				{
					Report.Info("Clicking 'Log In' on the Landing Page");
					var SelLandingPage = new LandingPage();
					SelLandingPage.Click_Login();
				...
				var NewUser = (User)SafewareSeleniumUtilities.Context.GetFromContext(savedAs);
				Report.Info("Entering Email: '" + NewUser.Email + "'");
				Sel_Login.Email_Field = NewUser.Email;
				Report.Info("Entering Password: '" + NewUser.Password + "'");
				Sel_Login.Password_Field = NewUser.Password;
				Report.Info("Clicking login");
				Sel_Login.Click_Login();
