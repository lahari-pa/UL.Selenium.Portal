...
namespace UL.Selenium.Portal.RPS.Steps
{
    ...
    class Steps_Login
    {
        ...
        public void LogInAsRetailer(string retailer)
        {
            ...
            Report.StartSubStep($"I login as retailer {retailer}");
            ...
            if (Report.IsTrue(user != null, $"Failed to find retailer {retailer}",
                $"Successfully found retailer {retailer}", true)) ;

            Report.IsTrue(new LandingPlatform().SignIn(user.UserName, user.Password), $"Failed to Log In as retailer {retailer}", $"Successfully Logged In as retailer {retailer}", true);
