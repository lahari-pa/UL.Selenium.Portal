...
namespace UL.Selenium.Portal.RPS.Steps
{
    ...
    public class Global_Steps
    {
        ...
        {
            if (Report.IsTrue(SeleniumBrowser.GetTabURLs().Contains(url), "Failed to find a tab with URL: " + url, "Successfully found a tab with URL: " + url, false, false))
            {
                var currUrl = SeleniumBrowser.WebBrowser.Url;
                SeleniumBrowser.SwitchToTabWithURL(url);
                Report.Screenshot();
                SeleniumBrowser.SwitchToTabWithURL(currUrl);
            }
        }
        ...
        {
            Report.IsTrue(SeleniumBrowser.GetTabURLs().Count == 1, "Another tab was found to be open!", "No tabs were found to be open!");
        }
        ...
        {
            SeleniumBrowser.WebBrowser.Navigate().GoToUrl(SeleniumBrowser.BaseTestUrl);
        }
        ...
        public void ConfirmNewFile(string savedAs)
        {
            ...
            while (timer < 30)
            {
                ...
                var differences = newFiles.Except(oldFiles).ToList();
                if (differences.Any())
                {
                    FileInfo mostRecent = differences.OrderByDescending(x => x.LastWriteTime).FirstOrDefault();
        ...
        public void CheckBitmapsAreTheSame(string file1, string file2)
        {
            ...
            try
            {
                imagesAreSame = GeneralFunctions.CompareImages(bitmap1, bitmap2, tolerancePercentage: 30);
        ...
        public void ICheckProductListBarCSVContainsValues(string fileSavedAs, string valuesSavedAs)
        {
            ...
            Report.IsTrue(supplierEmailsStr.Count == savedItems.ProductSuppliers.Count && !supplierEmailsStr.Except(savedItems.ProductSuppliers).Any(), "The Lists of Product Suppliers did not match", "The Lists of Product suppliers matched");
           ...
           // List<string> upcNumbersStr = new List<string>();
            for (int i = 1; i < lines.Count(); i++)
            {
                ...
                else
                {
                    ...
                    var supplier = Regex.Match(lineValues[1], "(.*)").Groups[1].Value.Trim();
                    var contact = Regex.Match(lineValues[3], "(.*)").Groups[1].Value.Trim();
                    if (contact == "")
                    {
                        Report.Info("");
                    }
                    var supplierEmail = Regex.Match(lineValues[7], "(.*)").Groups[1].Value.Trim();
                    //var upc = Regex.Match(lineValues[7], "(.*)").Groups[1].Value.Trim();
            ...
            var test = suppliersStr.Except(savedItems.Suppliers);
        ...
        public void ForSupplierSubscriptionStatusIPerformChartDrillDownExport()
        {
            ...
            Report.StartStep("I Confirm the Supplier List Is Showing");
            ...
            Report.StartStep("I Get the current Supplier List Items, save them to context as 'CurrentSupplierListItems' and then check they match the csv export saved as 'CurrentOutputCSV'");
        ...
        public void CloseBrowserTabWithTitle(string title)
        {
            ...
            while (i < 11)
            {
                ...
                foreach (string handle in allHandles)
                {
                    ...
                    if (currentTitle == title)
                    {
                        string url = SeleniumBrowser.GetActiveTabURL();
                        Report.IsTrue(SeleniumBrowser.CloseTabWithURL(url), "Failed to close tab with url: " + url,"Closed tab with url: " + url);
                        return;
                    }
                }
                Report.Info($"Did not find the tab:{title} on attempt: {i}");
