...
Given For product saved as: ID I should see an alert with the following message: Product Message for product ID has been created successfully
And I close alert
Given For product saved as: ID there should be a new email for email Address saved as: last-frozen.kxxyxunf@mailosaur.io from: noreply@wercsmart.ul.com with the title: Notification - Product <ID> - Document (Safety Data Sheet) is not Compliant with UN Globally Harmonized Standard (GHS)
