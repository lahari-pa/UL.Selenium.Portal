...
namespace UL.Selenium.Portal.WERCSmart.Philip
{
	...
	class StepDefinitions
	{
		...
		public void GivenIConfirmTheConsentToTierDataShowsTheAnswerAccept(string answer)
		{
			...
			Report.IsTrue(webElementsObject.ConfirmTierDataShowsCorrectAnswer(answer), "a", "b");
		}
		...
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.ClickCloseButtonInFunctionalPurposeDropdownMenu(), "a", "b");
		}
		...
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.ConfirmTheFollowingFunctionalPurposeIsDisplayed(functionalPurpose), "a", "b");
		}
		...
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.SelectTheFollowingFunctionalPurpose(functionalPurpose), "a", "b");
		}
		...
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.ConfirmDropDownMenuOpensInIngredientsPage(displayOrNotDisplayed), "a", "b");
		}
		...
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.SelectChooseOptionForFunctionalPurposeInIngredientsPage(), "a", "b");
		}
		...
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.FindReportDownloadPopupWithTheFollowingText(text), "Failed to find the correct text in the popup", "Successfully founded the correct text in the popup");
		}
		...
		[StepDefinition(@"I click the Download button for the follow report: (.*)")]
		public void ThenIClickTheDownloadButtonForTheMostRecentReport(string reportName)
		{
			WebElements webElementsObject = new WebElements();
			Report.IsTrue(webElementsObject.SelectDownloadButtonForMostRecenReport(reportName), "Failed to select Download button", "Successfully selected Download button");
			Delay.Seconds(10);
		}
