...
namespace UL.Selenium.Portal.WERCSmart.Philip
{
	class WebElements : SeleniumBaseObject
	{
		...
		{
			IWebElement answerEl = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//h3[text()='Consent to Tier 2.1, 2.2, 4.2 Data Uses']/following-sibling::p[text()='Accept']"), 2);
			if (answerEl != null)
			{
				return true;
			}
		...
		{
			IWebElement closeButton = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//div[@class='select2-link2 select2-close']//button"), 2);
			return closeButton.TryClick();
		}

		public bool ConfirmTheFollowingFunctionalPurposeIsDisplayed(string functionalPurpose)
		{
			IList<IWebElement> functionalPurposesEl = SeleniumBrowser.WebBrowser.FindElements(By.XPath("//ul[@class='select2-selection__rendered']//li"), 2);
		...
		{
			IWebElement functionalPurposeEl = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//li[@role='treeitem'][text()='" + functionalPurpose + "']"), 2);
			return functionalPurposeEl.TryClick();
		}

		public bool ConfirmDropDownMenuOpensInIngredientsPage(string displayOrNotDisplayed)
		{
			IWebElement dropDownMenu = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//ul[@class='select2-results__options']"), 2);
		...
		{
			IWebElement chooseOption = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//li[@class='select2-selection__choice']"), 2);
		...
		{
			IWebElement textEl = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//h3[text()='Report Download']/../following-sibling::div//div[@id='report-downloading-message']//p"), 2);
			Report.Info("testing1 " + textEl.Text);
			if (textEl.Text == text)
			{
				return true;
			}
		...
		public bool SelectDownloadButtonForMostRecenReport(string reportName)
		{
			Delay.Seconds(10);
			IWebElement downloadButton = SeleniumBrowser.WebBrowser.FindElement(By.XPath("//tbody[@data-bind='foreach: model.reportHistories']//tr//td[text()='" + reportName + "']/..//td[6]//button[1]"), 2);
		...
		{
			Delay.Seconds(10);
			IList<IWebElement> fileList = SeleniumBrowser.WebBrowser.FindElements(By.XPath("//tbody[@data-bind='foreach: model.reportHistories']//tr//td[text()='" + reportName + "']/.."), 2);
