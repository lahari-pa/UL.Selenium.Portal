...
namespace UL.Selenium.Portal.WERCSmart.Steps.SpecFlowSteps.New_Product.Product_Characteristics
{
	...
	class WERCSmart_Distributor_NewProducts_VOCOzoneTransportCommission
	{
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Amount of VOC content as weight percentage of the total formula, excluding exempt compounds as defined by the OTC Model Rule': to: (.*)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Would you like to use the VOC percentages entered for all areas \(e.g. country, state, local\) for comparison\?': to: (Yes|No, I would like to manually enter VOC value for each area.)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'HVOC \(high volatile organic compound\) content as weight percent of the total formulation': to: (.*)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'MVOC \(medium volatile organic compound\) content as weight percentage of the total formulation': to: (.*)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'VOC content in grams ozone per gram': to: (.*)")]
		public void SelectVOCPerGram(string option)
		{
			string section = "VOC content in grams ozone per gram";
			new Steps_Prototype().SetTheSectionOptionTo(section, option);
		}
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Product does not contain more than 0.05 grams of VOC per use, as defined in the California Consumer Products Regulation, Title 17, CCR Division 3, Chapter 1.': to: (Agree|Disagree)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Product label specifies a dilution ratio which results in a final VOC concentration for the product during use': to: (Yes|No)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Product's VOC content as sold': to: (.*)")]
		public void EnterVOCContentAsSold(string option)
		{
			string section = "Product's VOC content as sold";
			new Steps_Prototype().SetTheSectionOptionTo(section, option);
		}
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Product's VOC content as used': to: (.*)")]
		public void EnterVOCContentAsUsed(string option)
		{
			string section = "Product's VOC content as used";
			new Steps_Prototype().SetTheSectionOptionTo(section, option);
		}
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission section, I confirm text 'Please be sure you have selected the correct product type. For further questions, please contact Support.' (should|should not) be displayed")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Verify VOC content is below the threshold of 0.02lb/start of CARB': to: (Yes|No)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: 'Verify VOC content is below the threshold of 0.02lb/start of OTC': to: (Yes|No)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission section, I confirm text 'Need help\? Regulatory services are included in Premium Subscription. Upgrade now!' (should|should not) be displayed")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: HVOC \(high volatile organic compound\) content as weight percent of the total formulation (should|should not) be showing the error message: (.*)")]
		...
		[RegexStepDefinition(@"In the VOC - Ozone Transport Commission Section, set the option in section: MVOC \(medium volatile organic compound\) content as weight percentage of the total formulation (should|should not) be showing the error message: (.*)")]
